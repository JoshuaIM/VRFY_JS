...
console.log(resp[vl]['data'][0]['data']);
						...
						//---- 시계열 차트 시작
							...
							// 데이터가 하나 일 경우 series의 name이 제대로 기입이 안되므로 억지로 집어넣어준다.
								...
								if( var_select == "POP" ) {
									...
									// // TODO: 2021-06-01 소수점 둘째자리 반올림
									// var chtdata = new Array();
									// var new_d = resp[vl]['data'][0]['data'];
									// 	for(var x=0; x<new_d.length; x++) {
									// 		chtdata.push( parseFloat( new_d[x].toFixed(1) ) );
									// 	}
								}
                            	...
                            	for(var mm=0; mm<resp[vl]['data'].length; mm++) {
                                	...
                                	if( cht_arr_num[mm] < 1 ) {
        								...
        								if( var_select == "POP" ) {
											...
											// // TODO: 2021-06-01 소수점 둘째자리 반올림
											// var chtdata = new Array();
											// var new_d = resp[vl]['data'][mm]['data'];
											// 	for(var x=0; x<new_d.length; x++) {
											// 		chtdata.push( parseFloat( new_d[x].toFixed(1) ) );
											// 	}
        								}
