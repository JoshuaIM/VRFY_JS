...
var currentEndDate = "2018-08-01";
    ...
    function getDataArray() {
    	...
    	if( userSpecificPoints.length > 0 ) {
    		callAjaxArbiGrph(data_head, peri, var_select, init_hour, model_sel, start_init, end_init, vrfy_idx, map_type);
    	}
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//  임의기간 자료  Ajax 이용 그래픽 표출 메서드.
	function callAjaxArbiGrph(data_head, peri, var_select, init_hour, model_sel, start_init, end_init, vrfy_idx) {
    	$.ajax({
            type : "POST",
                data :
                {
                ...
                dataType: "json",
                url : '<?php echo site_url();?>/arbitraryExpression/getGrdArbiGrph',
    			// 변수에 저장하기 위함.
                async: false,
                success : function(resp) {

console.log("Ajax 시작 - resp.length: " + resp.length);
console.log( resp );
					...
					for(var vl=0; vl<resp.length; vl++) {
						...
						var selCont = "";
                     	...
                     	var vrfy_name = get_vrfy_title(vrfy_data, vrfy_title, dinfo_arr[1]);
    						selCont += "<h5><b class='chartName'>" + v_name + "_" + vrfy_name + " [" + resp[vl]['point_title'] + "] " + resp[vl]['rangeInfo'] + "</b></h5>";
    					...
    					//---- 집계표 테이블 시작
                 			selCont += "<table class='fcstTable'>";
             					selCont += "<tr class='tb_head'>";
            	    					selCont += "<td class='td_lg'>UTC</td><td class='td_lg'>모델 - 기법</td>";
    							...
    							for(var mm=0; mm<resp[vl]['xyData'].length; mm++) {
									...
									var utc = infoArr[1];
									var modl = infoArr[2] + "_" + infoArr[3];
        							
    								selCont += "<td class='tdinfo_txt'>" + utc + "</td><td class='tdinfo_txt'>" + modl + "</td>";
...
<div id="sidebar_sub">
	...
	<div class="js1-panel pn">
		...
		<table id="CHECK_SELECT" class="chk_select">
			...
			<tr>
				<td><input type="checkbox" name="INIT_HOUR" value="00#12" > 00UTC + 12UTC</td>
			</tr>
            -->
            </tbody>
		</table>
