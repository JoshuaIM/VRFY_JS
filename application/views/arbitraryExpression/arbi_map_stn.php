	...
	function callAjaxArbiData(data_head, peri, var_select, init_hour, model_sel, start_init, end_init, vrfy_idx) {
    	...
    	$.ajax({
            type : "POST",
                ...
                dataType: "json",
                url : '<?php echo site_url();?>/arbitraryExpression/getArbiStnFcstNum',
    			// 변수에 저장하기 위함.
                async: true,
                success : function(resp)
                {

					if( resp['fcst_info'] != null ) {
                ...
                for(var d=0; d<resp['date_info'].length; d++) {
            		grph_div += "<div class='img_area col-lg-3' >";
                		grph_div += "<div class='map_header'>";
							...
							// TODO: 2021-04-02 >> 임의기간에는 ymInfo 와 utcInfo 가 없었음. ymInfo 는 아예 없고, utcInfo는 utc_info로 사용되고 있음.
                			// grph_div += model_sel[m] + "_" + vrfy_name + "_" + resp['date_info'][d]['ymInfo'] + "_" + resp['date_info'][d]['utcInfo'];
                			grph_div += model_sel[m] + "_" + vrfy_name + "_" + resp['date_info'][d]['utc_info'];
                		grph_div += "</div>";
                		grph_div += "<div class='map_content'>";
                			...
                			grph_div += "<img class='grph_img' src='" + "<?php echo base_url('GIFD/');?>" + dt_arr[1] + "/TEMP/STN/" + var_select + "/" + nameOfgrph + "' onerror='no_image(this);' />" ;
...
<div id="sidebar_sub">
	...
	<div class="js1-panel pn">
		...
		<table id="CHECK_SELECT" class="chk_select">
			<tr>
				<td><input type="checkbox" id="00utc" name="INIT_HOUR" value="00#00" onclick="setHour(this.id);" checked > 00UTC (09KST)</td>
			</tr>
			<tr>
				<td><input type="checkbox" id="12utc" name="INIT_HOUR" value="12#12" onclick="setHour(this.id);" > 12UTC (21KST)</td>
			</tr>
			<tbody>
            </tbody>
		</table>
