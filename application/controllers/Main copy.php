...
class Main extends CI_Controller {
    ...
    private $shrt_dir = "SHRT/VRFY/MOND";
    private $medm_dir = "MEDM/VRFY/MOND";
    private $data_dir_type = "VRFY/MOND";

    private $bangjae_shrt_dir = "SHRT/VRFY/BNGJ";
    private $bangjae_medm_dir = "MEDM/VRFY/BNGJ";
    private $bangjae_season = [
        "05" => ["0515", "1015"],
        "11" => ["1115", "0315"]
    ];
...
//======= 지점: 단기 시계열 + 집계표   ===========================
    public function shrt_ts_stn($page = 'ts_stn'){
        ...
        $dataPath = $this->datafile_dir . $this->shrt_dir; 
        $data_to_template['dataDate'] = $this->common_func->getDirectoryDate($dataPath);

        // 방재기간(여름철, 겨울철) 추가
        // foreach( $this->bangjae_arr as $br ) {
        //     $data_to_template[$br] = $this->bangjae_func->getBangjaeYear("SHRT", $br);
        // }
        $data_to_template['bangjaeDate'] = $this->common_func->getDateDirectoryArray($this->datafile_dir . $this->bangjae_shrt_dir);
        
        $bangjae_date = $this->common_func->getDateDirectoryArray($this->datafile_dir . $this->bangjae_shrt_dir);
...
//======= 지점: 중기 시계열 + 집계표  ===========================
    public function medm_ts_stn($page = 'ts_stn'){
        ...
        $dataPath = $this->datafile_dir . $this->medm_dir;
        $data_to_template['dataDate'] = $this->common_func->getDirectoryDate($dataPath);
        ...
        $data_to_template['varName'] = $varName;
        // $data_to_template['varArray'] = ["T3H", "TMX", "TMN", "SKY", "PTY", "POP", "R12", "S12"];
        // $data_to_template['varnameArray'] = ["기온", "최고기온", "최저기온", "하늘상태", "강수유무", "강수확률", "12시간 강수량", "12시간 적설"];
        $data_to_template['varArray'] = ["T3H", "TMX", "TMN", "REH", "VEC", "WSD", "SKY", "PTY", "POP", "RN1", "SN1"];
        $data_to_template['varnameArray'] = ["기온", "최고기온", "최저기온", "습도",  "풍향", "풍속", "하늘상태", "강수유무", "강수확률", "시간 강수량", "시간 적설"];
        $data_to_template['vrfyTech'] = $this->common_func->getVrfyTech($varName);
...
public function getStnBangjaeData() {
    ...
    $bangjae_date = $this->input->post('bangjae_date');
    $vrfy_idx = $this->input->post('vrfy_idx');
    
    $fdir = $this->datafile_dir . $this->bangjae_shrt_dir;
    
    $rangeMon = $this->bangjae_func->getDateBangjae($bangjae_date, $this->bangjae_season, $init_hour);
...
// 공간분포 표출 시 예측시간 정보를 데이터로부터 가져오기(변수별로 예측시간 정보가 다르다).
    public function getStnFcstNum() {
        ...
        // SHRT or MEDM
        $dtype = $dhead_arr[1];
        $fdir = $fd . $dtype . "/" . $this->data_dir_type;
